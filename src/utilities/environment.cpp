...
namespace environment {
	...
	void environment::parseFolders(const std::filesystem::path folders[], const int numberOfFolders)
	{
		...
		{
			std::filesystem::path currentFolder = folders[index];
			std::filesystem::path currentFolderName = currentFolder.filename();
			...
			for (const auto& entry : std::filesystem::directory_iterator(currentFolder)) {
				...
				// file name
				counter::increaseValue(currentFolderName.string(), fileType);
				std::string fileName = std::to_string(counter::getValue(currentFolderName.string(), fileType));

				// file destination
				std::filesystem::path fileDestination(paths::greenlog::gBase / currentFolderName / fileType);
